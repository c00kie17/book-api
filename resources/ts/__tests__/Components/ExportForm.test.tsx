...
describe("ExportForm Component", () => {
    ...
    test("renders form in modal when isOpen is true", () => {
        ...
        expect(screen.getByTestId("mock-modal")).toHaveAttribute("data-title", "Export Books");


        expect(screen.getByTestId("radio-group")).toBeInTheDocument();
        expect(screen.getByTestId("radio-group")).toHaveAttribute("data-label", "Export Format");


        expect(screen.getByTestId("radio-button-format-csv")).toBeInTheDocument();
        expect(screen.getByTestId("radio-button-format-csv")).toHaveAttribute("data-checked", "true");


        expect(screen.getByTestId("radio-button-format-xml")).toBeInTheDocument();
        expect(screen.getByTestId("radio-button-format-xml")).toHaveAttribute("data-checked", "false");


        expect(screen.getByTestId("checkbox-group")).toBeInTheDocument();
        expect(screen.getByTestId("checkbox-group")).toHaveAttribute("data-label", "Fields to Export");

        expect(screen.getByTestId("checkbox-field-title")).toBeInTheDocument();
        expect(screen.getByTestId("checkbox-field-title")).toHaveAttribute("data-checked", "true");

        expect(screen.getByTestId("checkbox-field-author")).toBeInTheDocument();
        expect(screen.getByTestId("checkbox-field-author")).toHaveAttribute("data-checked", "true");
    });
})
